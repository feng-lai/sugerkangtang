...
use think\Exception;
use app\api\logic\common\UnionOrderRefundLogic;
 ...
 * Time: 17:31
 */
class UnionOrderRefund extends Api
{
  ...
  public function save()
  {
    ...
    if (!$request['price']) {
      throw new Exception('退回金额不能为空', 400);
    }

    if ($request['port'] == 'user') {
      $this->userInfo = $this->miniValidateToken();
    }
    $result = UnionOrderRefundLogic::commonAdd($request, $this->userInfo);
    if (isset($result['msg'])) {
