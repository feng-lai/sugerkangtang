 ...
 * 报名导出-控制器
 */
class OrderExport2 extends Api
{
    ...
    public function index()
    {
        ...
        $name = \app\api\model\Course::build()->where('uuid', $request['course_uuid'])->value('name');
        $result = OrderLogic::cmsList($request, $this->userInfo);
        $result = $result->toArray();

        $data = ['姓名', '学号', '书院', '专业', '报名状态','签到情况','签到数据','签到时间'];
        ...
        foreach ($result['data'] as $k => $v) {
            ...
            $resultPHPExcel->getActiveSheet()->setCellValue('A' . $a, $v['user_name']);
            ...
            $resultPHPExcel->getActiveSheet()->setCellValue('E' . $a, $v['order_status'] == 1?'报名成功':'报名取消');
            $resultPHPExcel->getActiveSheet()->setCellValue('F' . $a, $v['is_sign'] == 0?'未签到':'已签到');
            $resultPHPExcel->getActiveSheet()->setCellValue('G' . $a, $v['sign_num'].'/'.$v['order_num']);
        ...
        try {
            ...
            $file_name = mb_substr($name,0,5).'_'.date('Ymd').'_报名信息.xlsx';
