 ...
 * 出品方-校验
 */
class Producer extends Validate
{
    protected $rule = [
    ...
    protected function checkName($value, $rule, $data){
        ...
        if(\app\api\model\Producer::where($where)->find()){
            return '名称已存在';
        }
        return true;
    }
    ...
    protected function checkUname($value, $rule, $data){
        ...
        if(isset($data['uuid']) && $data['uuid']){
            $admin_uuid = \app\api\model\Producer::build()->where('uuid',$data['uuid'])->value('admin_uuid');
