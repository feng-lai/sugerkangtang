...
class Region extends Validate
{
    protected $rule = [
        ...
        'dealer_uuid'=>'require|checkProducerUuid',
    ];
    ...
    protected function checkName($value, $rule, $data){
        ...
        if(\app\api\model\Region::where($where)->find()){
            return '名称已存在';
        }
        return true;
    }
    ...
    protected function checkUname($value, $rule, $data){
        ...
        if(isset($data['uuid']) && $data['uuid']){
            $admin_uuid = \app\api\model\Region::build()->where('uuid',$data['uuid'])->value('admin_uuid');
    ...
    protected function checkProducerUuid($value, $rule, $data){
        if(!\app\api\model\Dealer::build()->where('uuid',$value)->where('is_deleted',1)->count()){
            return ['msg'=>'特邀经销商uuid不存在，请检查参数是否正确'];
        }
        return true;
    }
}
