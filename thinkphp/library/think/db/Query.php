...
class Query
{
    ...
    protected function throwNotFound($options = [])
    {
        if (!empty($this->model)) {
            ...
            throw new ModelNotFoundException('数据已删除或者找不到对应数据');
            //throw new ModelNotFoundException('数据已删除或者找不到对应数据:' . $class, $class, $options);
        } else {
            $table = is_array($options['table']) ? key($options['table']) : $options['table'];
            throw new DataNotFoundException('找不到对应表:' . $table, $table, $options);
        }
    }
